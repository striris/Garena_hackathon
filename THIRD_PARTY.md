...
| `openai/gpt-oss-120b` | Saltkin strategy-card and Director candidate selection | Model access and usage are supplied by the configured gateway |
...
Requests to the configured inference gateway contain aggregate resolved game
state, exactly three trusted Saltkin cards, pre-validated Director candidate
summaries with exact affected tile IDs, or a non-identifying player profile.
